...
export const approveVendorVehicleRequest = async (req, res, next) => {
  try {
    ...
    if (!approvedVendor) {
      next(errorHandler(500, "something went wrong while approvering vendor"));
    }

    res.status(200).json(approvedVendor);
  } catch (error) {
    console.log(error);
    next(errorHandler(500, "error while approvering vendor"));
  }
};
...
export const rejectVendorVehicleRequest = async (req, res, next) => {
  try {
    ...
    if (!regectedVendor) {
      next(errorHandler(500, "something went wrong while rejecting vendor"));
    }
